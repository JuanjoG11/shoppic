...
const ADMIN_PASSWORD = 'shoppic2024'; // Change this!
const VERCATALOGO_API = 'https://us-central1-mi-catalogo-1f031.cloudfunctions.net/api/variedadestvcali/product';

// State - clone from loaded products.js
let adminProducts = (typeof products !== 'undefined') ? JSON.parse(JSON.stringify(products)) : [];
let categories = [...new Set(adminProducts.map(p => p.category).filter(Boolean))];
if (!categories.length) categories = ['Hogar', 'Tecnología', 'Niños', 'Aseo'];
...
function updatePrice(input) {
    ...
    if (p) { p.price = newPrice; toast('Precio actualizado (sin guardar aún)', 'success'); }
}
...
function saveAllChanges() {
    const jsContent = `const products = ${JSON.stringify(adminProducts, null, 4)};\n`;
    ...
    toast('✅ products.js descargado. Reemplaza el archivo en la carpeta js/', 'success');
}
...
function saveEditProduct() {
    ...
    p.description = document.getElementById('edit-description').value.trim();
    p.image = document.getElementById('edit-image').value.trim();
    closeEditModal();
    renderProductsTable();
    toast('✅ Producto actualizado (recuerda guardar)', 'success');
}
...
function addNewProduct() {
    ...
    adminProducts.unshift({ id: newId, title, price, category, image, description });
    updateStats();
    renderProductsTable();
    renderCatStats();
    // Clear form
    ...
    updatePreview();
    toast('✅ Producto agregado al catálogo (recuerda guardar)', 'success');
...
function deleteCategory(cat) {
    const count = adminProducts.filter(p => p.category === cat).length;
    if (count > 0 && !confirm(`Esta categoría tiene ${count} producto(s). ¿Eliminarla de todas formas?`)) return;
